...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer,
                   ...
                   const int sensor,ConfigParam* pParams, IFrameSubscriber *sub, IMapPublisher *pub , bool bReuseMap):
    mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpORBVocabulary(pVoc),
    mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer*>(NULL)), mpSystem(pSys),
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
  //  mpFrameSubscriber(sub),mpMapPublisher(pub)
#else
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer,
                   Map *pMap, KeyFrameDatabase* pKFDB, const string &strSettingPath,
                   const int sensor,ConfigParam* pParams, IFrameSubscriber *sub, IMapPublisher *pub):
    mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpORBVocabulary(pVoc),
    mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer*>(NULL)), mpSystem(pSys),
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
  //  mpFrameSubscriber(sub),mpMapPublisher(pub)
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const std::vector<IMUData> &vimu, const double &timestamp)
{
    mvIMUSinceLastKF.insert(mvIMUSinceLastKF.end(), vimu.begin(),vimu.end());
    mImGray = imRGB;
    cv::Mat imDepth = imD;
    ...
    mCurrentFrame = Frame(mImGray,imDepth,timestamp,vimu,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                if(mpLocalMapper->GetVINSInited())
                {
                    ...
                    {
                       // cout<<"timestamp gap: "<<mCurrentFrame.mTimeStamp-mLastFrame.mTimeStamp<<endl;
                        bOK = TrackWithIMU(bMapUpdated);
                       // cout<<"Tracking with IMU is "<<bOK<<endl;
                        if(!bOK){
 ...
 if(mState==LOST)
        ...
        if(bOK)
        {
            ...
            if(mbRelocBiasPrepare)
            {
                ...
                {
                    NavState nscur;
                    //std::thread threadObj(&Tracking::RecomputeIMUBiasAndCurrentNavstate, this, std::ref(nscur)); 
                    //threadObj.join();
...
void Tracking::StereoVIOInitialization()
{
    if(mCurrentFrame.N>500)
    {
        ...
        // Create KeyFrame
        KeyFrame* pKFini = new KeyFrame(mCurrentFrame,mpMap,mpKeyFrameDB,simu,NULL);
        ...
        cout << "New map created with " << mpMap->MapPointsInMap() << " points" << counte << endl;
...
void Tracking::CreateNewKeyFrame_reUse()
{
    if(!mpLocalMapper->SetNotStop(true))
        return;
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
        ...
        if(pPrevKF)
        {
            if(pPrevKF->isBad()) {cerr<<"pPrevKF is bad in UpdateLocalKeyFrames()?????"<<endl;return;}
        ...
        if(pNextKF)
        {
            if(pNextKF->isBad()) {cerr<<"pNextKF is bad in UpdateLocalKeyFrames()?????"<<endl;return;}
