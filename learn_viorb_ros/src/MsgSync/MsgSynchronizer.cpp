...
namespace ORBVIO {
    ...
    bool MsgSynchronizer::getRecentMsgs(sensor_msgs::ImageConstPtr &imgmsg,
    ...
    bool MsgSynchronizer::getRecentMsgsStereo(sensor_msgs::ImageConstPtr &msgLeft, sensor_msgs::ImageConstPtr &msgRight,
                                              std::vector<sensor_msgs::ImuConstPtr> &vimumsgs) {
        //unique_lock<mutex> lock2(_mutexIMUQueue);
        unique_lock<mutex> lock1(_mutexImageQueueLeft);
        unique_lock<mutex> lock2(_mutexImageQueueRight);

        if (_status == NOTINIT || _status == INIT) {
            //ROS_INFO("synchronizer not inited");
            return false;
        }
        if (_imageMsgQueueLeft.empty()) {
            //ROS_INFO("no image stored in queue currently.");
            return false;
        }
        if (_imuMsgQueue.empty()) {
        ...
        {
            sensor_msgs::ImageConstPtr imsgLeft;
            sensor_msgs::ImageConstPtr imsgRight;
            sensor_msgs::ImuConstPtr bmsg;

            //
            imsgLeft = _imageMsgQueueLeft.back();
            imsgRight = _imageMsgQueueRight.back();
            bmsg = _imuMsgQueue.front();

            // Check dis-continuity, tolerance 3 seconds
            if (imsgLeft->header.stamp.toSec() - _imageMsgDelaySec + 3.0 < bmsg->header.stamp.toSec()) {
                ROS_ERROR("Data dis-continuity, > 3 seconds. Buffer cleared");
                clearMsgsStereo();
                return false;
            }
            ...
            //
            imsgLeft = _imageMsgQueueLeft.front();
            imsgRight = _imageMsgQueueRight.front();
            bmsg = _imuMsgQueue.back();

            // Wait imu messages in case communication block
            if (imsgLeft->header.stamp.toSec() - _imageMsgDelaySec > bmsg->header.stamp.toSec()) {
                return false;
            }

            // Check dis-continuity, tolerance 3 seconds
            if (imsgLeft->header.stamp.toSec() - _imageMsgDelaySec > bmsg->header.stamp.toSec() + 3.0) {
                ROS_ERROR("Data dis-continuity, > 3 seconds. Buffer cleared");
                clearMsgsStereo();
                return false;
            }
            ...
            // Wait until the imu packages totolly com
            if (_imageMsgQueueLeft.size() < 10 && _imuMsgQueue.size() < 15
                && imsgLeft->header.stamp.toSec() - _imageMsgDelaySec > bmsg->header.stamp.toSec()) {
                //ROS_WARN_STREAM("here return, last imu time "<<);
                return false;
        ...
        // get image message
        msgLeft = _imageMsgQueueLeft.front();
        msgRight = _imageMsgQueueRight.front();

     //   cv::imshow("left", cv_bridge::toCvShare(msgLeft)->image.clone());
     //   cv::imshow("right", cv_bridge::toCvShare(msgRight)->image.clone());
     //   cv::waitKey();

        _imageMsgQueueLeft.pop();
        _imageMsgQueueRight.pop();
        ...
        while (true) {
            ...
            // consider delay between image and imu serial
            sensor_msgs::ImuConstPtr tmpimumsg = _imuMsgQueue.front();
            if (tmpimumsg->header.stamp.toSec() < msgLeft->header.stamp.toSec() - _imageMsgDelaySec) {
                // add to imu message vector
                vimumsgs.push_back(tmpimumsg);
                {
                    unique_lock<mutex> lock(_mutexIMUQueue);
    ...
    void MsgSynchronizer::addImageMsgStereo(const sensor_msgs::ImageConstPtr &msgLeft,
                                            const sensor_msgs::ImageConstPtr &msgRight) {
        unique_lock<mutex> lock(_mutexImageQueueLeft);
        unique_lock<mutex> lock1(_mutexImageQueueRight);
        ...
        if (_imageMsgDelaySec >= 0) {
            ...
            if (_status == NOTINIT)
                ...
                // ignore all image messages with no imu messages between them
                // only add below images
                if (msgLeft->header.stamp.toSec() - _imageMsgDelaySec > _imuMsgTimeStart.toSec()) {
                    _imageMsgQueueLeft.push(msgLeft);
                    _imageMsgQueueRight.push(msgRight);
                    _status = NORMAL;
                }
            } else {
                // push message into queue
            ...
            if (_status == NOTINIT) {
                _imuMsgTimeStart = msgLeft->header.stamp;
                _status = INIT;
            } else {   // no image data if there's no imu message
                _imageMsgQueueLeft.push(msgLeft);
                _imageMsgQueueRight.push(msgRight);
    ...
    void MsgSynchronizer::imageCallbackStereo(const sensor_msgs::ImageConstPtr &msgLeft,
                                              const sensor_msgs::ImageConstPtr &msgRight) {

        addImageMsgStereo(msgLeft, msgRight);
    }
    ...
    void MsgSynchronizer::clearMsgsStereo(void) {
        _imuMsgQueue = std::queue<sensor_msgs::ImuConstPtr>();
        _imageMsgQueueLeft = std::queue<sensor_msgs::ImageConstPtr>();
        _imageMsgQueueRight = std::queue<sensor_msgs::ImageConstPtr>();
    }
