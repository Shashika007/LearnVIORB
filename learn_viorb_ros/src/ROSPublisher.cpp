...
#include "../msg_gen/cpp/include/ORB_VIO/odom_data.h" 
...
Q convertToQuaternion(const cv::Mat& rot)
{
	...
	tmp[k] = (rot.at<float>(k,i) + rot.at<float>(i,k)) * s;
      ...
      return {-tmp[2], tmp[0], tmp[1], -tmp[3]};
}
...
sensor_msgs::PointCloud2 convertToPCL2(const std::vector<MapPoint*> &map_points)
{
    ...
    //std::vector<uint8_t> data_buffer(n_map_points *2* sizeof(point));
    ...
    pub_rate_(frequency)
{
    ...
    odom_pub_ = nh.advertise<nav_msgs::Odometry>("vi_odom", 10);
}
...
void ROSPublisher::Run()
{
	...
	tf::Vector3 position = { xf.at<float>(2, 3), -xf.at<float>(0, 3), -xf.at<float>(1, 3) };
	...
	camera_tf_pub_.sendTransform(transform);
        odom_data.pose.pose.position.x = xf.at<float>(2, 3);
        odom_data.pose.pose.position.y = -xf.at<float>(0, 3);
        odom_data.pose.pose.position.z = -xf.at<float>(1, 3);
...
ROS_System::ROS_System(const std::string& strVocFile,
...
#endif
{
    mpPublisher = std::make_unique<ROSPublisher>(
...
ROS_System::~ROS_System() { }

IPublisherThread* ROS_System::GetPublisher()
{
    return mpPublisher.get();
}

IMapPublisher* ROS_System::GetMPublisher()
{
    return mpPublisher.get();
}

IFrameSubscriber* ROS_System::GetSubscriber()
{
    return mpPublisher.get();
}
